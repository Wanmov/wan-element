...
export const Group: Story & { args: { content1: string; content2: string } } = {
  ...
  render: (args) => ({
    components: { WanButton, WanButtonGroup },
    setup() {
      return { args };
    },
    template: container(`
       <wan-button-group :type="args.groupType" :size="args.groupSize" :disabled="args.groupDisabled">
         <wan-button v-bind="args">{{args.content1}}</wan-button>
         <wan-button v-bind="args">{{args.content2}}</wan-button>
       </wan-button-group>
    `),
  }),
  play: async ({ canvasElement, args, step }) => {
