...
import { isString, isFunction, each, set, isObject, assign } from "lodash-es";
...
function initInstance(props: MessageBoxProps, container: HTMLElement) {
  ...
  const vnode = createVNode(
    ...
    isVNodeMsg ? { default: genDefaultSlot(props.message as VNode) } : undefined
  );
...
function messageBoxFactory(boxType: (typeof MESSAGE_BOX_VARIANTS)[number]) {
  return (
    ...
    let titleOrOpts = isObject(title) ? "" : title ?? "";
    if (isObject(title)) options = title as MessageBoxOptions;
...
export default MessageBox as IWanMessageBox;
